...
async def predecir_individual(request: PrediccionRequest):
    ...
    **Nivel de riesgo:**
    ...
    try:
        logger.info(
            f"📊 Solicitud de predicción: {request.tipo_prediccion} para estudiante {request.estudiante_id}")
...
async def predecir_batch(request: PrediccionBatchRequest):
    ...
    try:
        logger.info(
            f"📊 Predicción en lote: {len(request.estudiantes_ids)} estudiantes")
        ...
        for estudiante_id in request.estudiantes_ids:
            try:
                ...
                logger.warning(
                    f"⚠️ Error en predicción de {estudiante_id}: {e}")
                errores.append({
...
async def entrenar_modelo(
    ...
    **Tipos de modelo disponibles:**
    ...
    try:
        logger.info(
            f"🎓 Iniciando entrenamiento de modelo: {request.tipo_modelo}")
