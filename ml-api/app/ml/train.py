...
class ModelTrainer:
    ...
    def cargar_datos(self):
        ...
        response_estudiantes = self.supabase.table(
            "estudiantes").select("*").execute()
        ...
        # Obtener asistencia
        response_asist = self.supabase.table(
            "asistencia").select("*").execute()
    ...
    def preparar_features(self, estudiantes_df, notas_df, asist_df, conducta_df):
        ...
        for _, estudiante in estudiantes_df.iterrows():
            ...
            porcentaje_aprobacion = len(
                notas_est[notas_est['nota'] >= 11]) / len(notas_est) * 100 if len(notas_est) > 0 else 100
            ...
            tasa_asistencia = (total_dias - ausencias) / \
                total_dias * 100 if total_dias > 0 else 100
            ...
            if len(cond_est) > 0:
                ...
                tiene_incidencias = 1 if pd.notna(
                    cond_est.iloc[-1].get('incidencias')) else 0
            else:
                conducta_score = 0.75
                tiene_incidencias = 0
        ...
        logger.info(
            f"✅ Features preparadas: {len(df)} muestras, {len(df.columns)-1} features")
    ...
    def entrenar_random_forest(self, X_train, X_test, y_train, y_test):
        ...
        model_path = os.path.join(
            settings.model_path, 'desaprobacion_classifier.pkl')
    ...
    def entrenar_xgboost(self, X_train, X_test, y_train, y_test):
        ...
        logger.info(
            f"✅ XGBoost - Accuracy: {metrics['accuracy']:.4f}, F1: {metrics['f1_score']:.4f}")

        # Guardar modelo
        model_path = os.path.join(
            settings.model_path, 'desercion_classifier.pkl')
    ...
    def entrenar_todos(self):
        ...
        logger.info(
            f"📊 Distribución de clases - En riesgo: {y.sum()}, Sin riesgo: {len(y) - y.sum()}")
